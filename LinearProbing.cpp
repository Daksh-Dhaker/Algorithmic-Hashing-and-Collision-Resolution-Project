...
void LinearProbing::merge1(std::vector<int> &v,int l,int mid,int r){
    std::vector<int> v1;
    std::vector<int> v2;
    for(int i=l;i<=mid;i++){
        v1.push_back(v[i]);
...
void LinearProbing::merge_sort(std::vector<int> &v,int l,int r){
    if(l==r){
        return;
    }else{
        int mid = (l+r)/2;
...
int LinearProbing::fastpower(int base,int expo){
    if(expo ==0){
        return 1;
    }else if(expo == 1){
        return base;
...
void LinearProbing::createAccount(std::string id, int count) {
    ...
    while(bankStorage1d[probe].id !="NULL"){
        i++;
        probe = (hash_idx+(i)%100000)%100000;
    }
